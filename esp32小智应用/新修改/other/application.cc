...
#ifdef CONFIG_BOARD_TYPE_MY_ESP32_BOARD
    auto_listen_requested_ = true;
#endif
}
...
#ifdef CONFIG_BOARD_TYPE_MY_ESP32_BOARD
    if (auto_listen_requested_) {
        auto_listen_requested_ = false;
        Schedule([this]() {
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnAudioChannelOpened([this, codec, &board]() {
        ...
        if (protocol_->server_sample_rate() != codec->output_sample_rate()) {
            ESP_LOGW(TAG, "Server sample rate %d != device output %d, resampling may cause distortion",
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (cJSON_IsString(text)) {
                ...
                // 调用板子的回调，用于命令匹配
                auto& board = Board::GetInstance();
                board.OnSttText(text->valuestring);
            }
...
void Application::ContinueOpenAudioChannel(ListeningMode mode) {
    ...
    // 确保音频处理器正在运行（解决休眠唤醒后失效）
    if (!audio_service_.IsAudioProcessorRunning()) {
        ESP_LOGW(TAG, "Audio processor not running, restarting audio service...");
        ...
        callbacks.on_wake_word_detected = [this](const std::string& w) { xEventGroupSetBits(event_group_, MAIN_EVENT_WAKE_WORD_DETECTED); };
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateIdle:
            ...
            display->SetEmotion("neutral");
            if (!audio_service_.IsAudioProcessorRunning()) {
                ESP_LOGW(TAG, "Audio processor not running, restarting...");
                auto codec = Board::GetInstance().GetAudioCodec();
                audio_service_.Stop();
                audio_service_.Initialize(codec);
                audio_service_.Start();
...
void Application::SetListeningMode(ListeningMode mode) {
    ...
    if (!audio_service_.IsAudioProcessorRunning()) {
        ESP_LOGW(TAG, "Audio processor not running, restarting before listen...");
        auto codec = Board::GetInstance().GetAudioCodec();
        audio_service_.Stop();
        audio_service_.Initialize(codec);
        audio_service_.Start();
...
void Application::InjectUserText(const std::string& text) {
    if (text.empty()) return;
    ESP_LOGI(TAG, "Inject user text: %s", text.c_str());
    Schedule([this, text]() {
            ...
            if (GetDeviceState() != kDeviceStateConnecting) {
                ...
                Schedule([this, text]() {
                    if (protocol_ && protocol_->IsAudioChannelOpened()) {
                        ...
                        cJSON_AddStringToObject(payload, "type", "user_text");
                        ...
                        ESP_LOGE(TAG, "Failed to open audio channel for injected text");
                    }
                });
            }
        } else {
...
// ========== TaskPriorityReset Implementation ==========
TaskPriorityReset::TaskPriorityReset(BaseType_t priority) {
    original_priority_ = uxTaskPriorityGet(NULL);
    vTaskPrioritySet(NULL, priority);
}

TaskPriorityReset::~TaskPriorityReset() {
