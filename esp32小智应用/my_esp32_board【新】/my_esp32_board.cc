        ...
        while (1) {
            ...
            if (len > 0) {
                for (int i = 0; i < len; i++) {
                    ...
                    if (c == '\n' || c == '\r') {
                        if (!line.empty()) {
                            ESP_LOGI(TAG, "UART line: %s", line.c_str());
                            if (line.rfind("Q:", 0) == 0) {
                                ...
                                Application::GetInstance().SendTextMessage(question,
                                    [](const std::string& answer) {
                                        std::string response = "A:" + answer + "\r\n";
                                        uart_write_bytes(UART_NUM_1, response.c_str(), response.length());
                                    });
                            } else {
                                // 非提问内容原样回传（或忽略）
                                std::string echo = "Echo: " + line + "\r\n";
                                uart_write_bytes(UART_NUM_1, echo.c_str(), echo.length());
                            }
                            line.clear();
                        }
                    } else {
