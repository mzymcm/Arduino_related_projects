        ...
        switch (event) {
            ...
            case NetworkEvent::Connecting: {
                if (data.empty()) {
                    ...
                    std::string msg = Lang::Strings::CONNECT_TO + data + "...";
                    display->ShowNotification(msg.c_str(), 30000);
                }
                break;
            }
            case NetworkEvent::Connected: {
                std::string msg = Lang::Strings::CONNECTED_TO + data;
                display->ShowNotification(msg.c_str(), 30000);
...
#if ALWAYS_LISTENING_MODE
    if (GetDeviceState() == kDeviceStateIdle) {
        Schedule([this]() {
            ESP_LOGI(TAG, "Network reconnected, auto start listening");
            StartListeningSync();
        });
    }
#endif
...
void Application::HandleActivationDoneEvent() {
    ...
    // 开机自动模拟按键，进入聆听状态
    // 使用 Schedule 延迟执行，确保状态机完全进入 idle 且协议已就绪
    Schedule([this]() {
        ESP_LOGI(TAG, "Auto trigger ToggleChatState to start listening (simulate boot key)");
        ToggleChatState();
    });
}
...
void Application::CheckAssetsVersion() {
    ...
    if (!download_url.empty()) {
        ...
        bool success = assets.Download(download_url, [this, display](int progress, size_t speed) {
            ...
            Schedule([display, msg = std::string(buffer)]() { display->SetChatMessage("system", msg.c_str()); });
        });
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnAudioChannelOpened([this, codec, &board]() {
        ...
        if (protocol_->server_sample_rate() != codec->output_sample_rate()) {
            ESP_LOGW(TAG, "Server sample rate %d != output %d, resampling may cause distortion",
...
protocol_->OnAudioChannelClosed([this, &board]() {
    ...
    Schedule([this]() {
        ...
        display->SetChatMessage("system", "");
#if ALWAYS_LISTENING_MODE
        // 通道关闭后，不立即重启，而是记录日志，等待 idle 状态处理
        ESP_LOGI(TAG, "Audio channel closed, will restart in idle state");
#else
        SetDeviceState(kDeviceStateIdle);
#endif
    });
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                Schedule([this]() { aborted_ = false; SetDeviceState(kDeviceStateSpeaking); });
            } else if (strcmp(state->valuestring, "stop") == 0) {
                Schedule([this]() {
                    if (GetDeviceState() == kDeviceStateSpeaking) {
#if ALWAYS_LISTENING_MODE
                        ESP_LOGI(TAG, "TTS stop, auto restart listening");
                        StartListeningSync();
#else
                        if (listening_mode_ == kListeningModeManualStop) SetDeviceState(kDeviceStateIdle);
                        else SetDeviceState(kDeviceStateListening);
#endif
                    }
                });
            ...
            if (cJSON_IsObject(payload)) McpServer::GetInstance().ParseMessage(payload);
            ...
            auto emotion = cJSON_GetObjectItem(root, "emotion");
            if (cJSON_IsString(status) && cJSON_IsString(msg) && cJSON_IsString(emotion)) {
                Alert(status->valuestring, msg->valuestring, emotion->valuestring, Lang::Sounds::OGG_VIBRATION);
            }
        } else {
...
void Application::ShowActivationCode(const std::string& code, const std::string& message) {
    ...
    static const std::array<digit_sound, 10> digit_sounds{{
        {'0', Lang::Sounds::OGG_0}, {'1', Lang::Sounds::OGG_1}, {'2', Lang::Sounds::OGG_2},
        {'3', Lang::Sounds::OGG_3}, {'4', Lang::Sounds::OGG_4}, {'5', Lang::Sounds::OGG_5},
        {'6', Lang::Sounds::OGG_6}, {'7', Lang::Sounds::OGG_7}, {'8', Lang::Sounds::OGG_8},
        {'9', Lang::Sounds::OGG_9}
    }};
    ...
    for (char digit : code) {
        ...
        if (it != digit_sounds.end()) audio_service_.PlaySound(it->sound);
    }
}
...
void Application::Alert(const char* status, const char* message, const char* emotion, const std::string_view& sound) {
    ...
    if (!sound.empty()) audio_service_.PlaySound(sound);
}
...
// 同步启动聆听（用于永远在线模式，避免事件循环延迟）
void Application::StartListeningSync() {
    if (!protocol_) {
        ESP_LOGE(TAG, "StartListeningSync: protocol not initialized");
        return;
    }
    DeviceState state = GetDeviceState();
    if (state == kDeviceStateIdle || state == kDeviceStateConnecting) {
        ESP_LOGI(TAG, "StartListeningSync: opening audio channel");
        ...
        ESP_LOGI(TAG, "StartListeningSync: state=%d, skip", state);
    }
}
...
void Application::HandleStopListeningEvent() {
    ...
    if (state == kDeviceStateAudioTesting) {
        ...
        if (protocol_) protocol_->SendStopListening();
        SetDeviceState(kDeviceStateIdle);
    }
}
...
void Application::HandleWakeWordDetectedEvent() {
    ...
    if (state == kDeviceStateIdle) {
        ...
        auto ww = audio_service_.GetLastWakeWord();
        if (!protocol_->IsAudioChannelOpened()) {
            SetDeviceState(kDeviceStateConnecting);
            Schedule([this, ww]() { ContinueWakeWordInvoke(ww); });
            return;
        }
        ContinueWakeWordInvoke(ww);
...
void Application::ContinueWakeWordInvoke(const std::string& wake_word) {
    ...
    while (auto packet = audio_service_.PopWakeWordPacket()) protocol_->SendAudio(std::move(packet));
...
#if ALWAYS_LISTENING_MODE
            // 永远在线模式：进入 idle 后自动重新开始聆听
            ESP_LOGI(TAG, "Idle state in always-listening mode, auto restart listening");
            // 使用 Schedule 延迟执行，避免递归
            Schedule([this]() {
                // 直接调用 ToggleChatState 模拟按键
                ToggleChatState();
            });
#endif
            break;
...
void Application::AbortSpeaking(AbortReason reason) {
    ...
    aborted_ = true;
    if (protocol_) protocol_->SendAbortSpeaking(reason);
}
...
ListeningMode Application::GetDefaultListeningMode() const {
    return kListeningModeRealtime;   // 永远在线使用实时流模式
}
...
void Application::SendMcpMessage(const std::string& payload) {
    Schedule([this, payload]() {
        ...
        if (mcp_broadcast_callback_) mcp_broadcast_callback_(payload);
    });
}
