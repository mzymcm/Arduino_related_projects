...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    ...
    for (int i = 0; i < samples; i++) {
        // 修正：INMP441 有效数据在高16位，右移16位得到16-bit值
        int32_t value = bit32_buffer[i] >> 16;
        dest[i] = (value > INT16_MAX) ? INT16_MAX : (value < -INT16_MAX) ? -INT16_MAX : (int16_t)value;
    }

    // 调试：每100次读取打印一次前10个样本的平均绝对值
    static int call_count = 0;
    if (call_count++ % 100 == 0 && samples > 0) {
        int sum_abs = 0;
        int num = samples < 10 ? samples : 10;
        for (int i = 0; i < num; i++) {
            sum_abs += abs(dest[i]);
        }
        ESP_LOGI(TAG, "Mic data avg(abs) first %d: %d", num, sum_abs / num);
    }
