...
bool MqttProtocol::SendUserText(const std::string& text) {
    // 1. 检查发布主题是否配置
    if (publish_topic_.empty()) {
        ESP_LOGW(TAG, "publish_topic is empty, cannot send user text");
        return false;
    }
    // 2. 检查 MQTT 客户端是否就绪
    if (!mqtt_ || !mqtt_->IsConnected()) {
        ESP_LOGW(TAG, "MQTT not connected, cannot send user text");
        return false;
    }

    // 3. 构建 JSON 消息
    cJSON* root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "type", "user_input");
    cJSON_AddStringToObject(root, "text", text.c_str());
    char* json_str = cJSON_PrintUnformatted(root);
    
    // 4. 使用正确的主题发送
    bool ret = mqtt_->Publish(publish_topic_, json_str);
    ...
    if (!ret) {
        ESP_LOGE(TAG, "Failed to publish user text to topic %s", publish_topic_.c_str());
    } else {
        ESP_LOGI(TAG, "User text sent: %s", text.c_str());
    }
    return ret;
}
...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    mqtt_->OnMessage([this](const std::string& topic, const std::string& payload) {
      ESP_LOGI(TAG, "MQTT received: %s", payload.c_str());   // 添加这行
