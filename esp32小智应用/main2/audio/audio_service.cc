...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    // 编码器配置：采样率使用 codec 的实际输入采样率（例如 24000）
    esp_opus_enc_config_t opus_enc_cfg = {
        .sample_rate = codec->input_sample_rate(),
        .channel = ESP_AUDIO_MONO,
        .bits_per_sample = ESP_AUDIO_BIT16,
    ...
    if (opus_encoder_ == nullptr) {
        ...
        encoder_sample_rate_ = codec->input_sample_rate();
    ...
    // 不再需要输入重采样器（因为编码器直接使用输入采样率）
    if (codec->input_sample_rate() != encoder_sample_rate_) {
        ESP_LOGW(TAG, "Input sample rate %d != encoder sample rate %d, resampling may be needed",
                 codec->input_sample_rate(), encoder_sample_rate_);
    }
...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        if (bits & AS_EVENT_AUDIO_TESTING_RUNNING) {
            ...
            int samples = OPUS_FRAME_DURATION_MS * encoder_sample_rate_ / 1000;
        ...
        if (bits & (AS_EVENT_WAKE_WORD_RUNNING | AS_EVENT_AUDIO_PROCESSOR_RUNNING)) {
            // 关键修改：使用编码器帧大小，确保与编码器匹配
            int samples = encoder_frame_size_;
            std::vector<int16_t> data;
            if (ReadAudioData(data, encoder_sample_rate_, samples)) {
